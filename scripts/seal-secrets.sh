...
for env_file in "${env_files[@]}"; do
  ...
  if [[ "$name" == "elasticsearch" ]]; then
    ...
    file_realm_out="$sealed_dir/${file_realm_name}-sealedsecret.yaml"
    ...
    if [[ "$elastic_password" =~ ^\$2[aby]\$ ]]; then
      elastic_line="elastic:${elastic_password}"
    else
      if ! command -v htpasswd >/dev/null 2>&1; then
        echo "htpasswd not found in PATH. Needed for bcrypt hashing." >&2
        exit 2
      fi
      elastic_line="$(htpasswd -nbB elastic "$elastic_password" | tr -d '\r')"
    fi

    users_lines=()
    users_lines+=("$elastic_line")
    users_list=("elastic")
    ...
    while IFS='=' read -r key value; do
      ...
      if [[ "$key" == *_ES_USER ]]; then
        user_name="$value"
        pass_key="${key%_ES_USER}_ES_PASS"
        if user_pass=$(get_env_value "$env_file" "$pass_key" 2>/dev/null); then
          if [[ "$user_pass" =~ ^\$2[aby]\$ ]]; then
            users_lines+=("${user_name}:${user_pass}")
          else
            users_lines+=("$(htpasswd -nbB "$user_name" "$user_pass" | tr -d '\r')")
          fi
    ...
    printf "superuser:%s" "$(IFS=,; echo "${users_list[*]}")" > "$roles_file"
